...
patricia* patricia::maxmatch(char* _key, const patricia* last) const
{
	if ( _bitncmp(_key,str,len) ) {
		printf("entrou em eu nao sou uma substrnig da chave\n");
		/* If I am not a substring of the _key */
		return (patricia*)last;
	} else if ( _bitlen(_key) == len ) {
		printf("entrou em eu sou exatamente a chave\n");
		/* If I am exactly the _key */
		return (patricia*)this;
	} else {
		printf("entrou em eu sou uma substing da chave\n");
...
patricia* patricia::_search(char* _key, const patricia* last)
{
	if ( _bitncmp(_key,str,len) ) {
		...
		if(found == 0){
			found = 1;
			//printf("a palavra %s foi marcada\n", _key);
		}else{
			printf("a palavra %s ja foi marcada\n", _key);
		}
		*/
